...
class ProfileView extends React.Component<IProfileViewProps, IProfileViewState> {
	...
	submit = async (): Promise<void> => {
		...
		// 				description={I18n.t('Two_Factor_Success_message')}
		...
		try {
			const twoFactorOptions = params.currentPassword
				...
				: null;
			console.log('params, customFields, temp || twoFactorOptions', params, customFields, code || twoFactorOptions);
			const result = await Services.saveUserProfileMethod(params, customFields, code || twoFactorOptions);

			try {
				const userResult = await Services.getUserInfo(user?.id);
				if (userResult?.success) {
					dispatch(setUser({ ...(userResult.user as IUser) }));
					this.setState({ saving: false });
				}
			} catch (error) {
				log(error);
